...
class LaporanApiController extends Controller
{
    ...
    /**
     * Semua laporan dari semua warga (untuk halaman "Semua Laporan").
     */
    ...
    /**
     * Buat laporan baru dari aplikasi Flutter.
     */
    public function store(Request $request)
    {
        ...
        Notifikasi::create([
            ...
            'pesan'       => "Laporan \"{$laporan->judul}\" telah dikirim dan menunggu verifikasi.",
            'tipe'        => 'laporan_baru',
            'sudah_dibaca'=> false,
    ...
    private function format(Laporan $l, bool $withUser = false, bool $withPenugasan = false): array
    {
        $data = [
            ...
            'foto_sebelum' => $l->foto_sebelum
                ? asset('storage/' . $l->foto_sebelum)
                : null,
            'foto_sesudah' => $l->foto_sesudah
                ? asset('storage/' . $l->foto_sesudah)
                : null,
            'status'       => $l->status,
        ...
        if ($withUser && $l->relationLoaded('user')) {
            $data['nama_warga'] = $l->user?->name;
        }

        if ($withPenugasan && $l->relationLoaded('penugasan') && $l->penugasan) {
            $data['penugasan'] = [
                'petugas'        => $l->penugasan->petugas?->nama,
                'status'         => $l->penugasan->status,
                'ditugaskan_pada'=> $l->penugasan->ditugaskan_pada?->toIso8601String(),
